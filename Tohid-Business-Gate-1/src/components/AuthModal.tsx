...
export const AuthModal = ({ isOpen, onClose }: { isOpen: boolean, onClose: () => void }) => {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-[#1B5E3F]/40 backdrop-blur-md p-4">
      <Card className="max-w-md w-full glass-card rounded-[2.5rem] shadow-2xl relative border-2 border-white/50">
        ...
        <CardHeader className="text-center pt-10">
          <CardTitle className="text-3xl font-bold">{mode === 'login' ? 'Welcome Back' : 'Join OmanGate'}</CardTitle>
        ...
        <CardContent className="space-y-6 pb-12">
          ...
          <form onSubmit={handleEmailAuth} className="space-y-4">
            ...
            <Button className="w-full h-14 rounded-2xl bg-primary text-lg font-bold" disabled={loading}>
